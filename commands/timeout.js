 ...
 */
  async execute(client, interaction) {
    ...
    if (totalSeconds === 0) {
      member.timeout(null, "Timeout Removed | Removed by " + interaction.user.tag);
  }
    if(member.manageable || member.moderatable) {
      member.timeout(totalSeconds * 1000, `${reason} | Issued by: ${interaction.user.tag}`);
      ...
      .setDescription(`${user.tag} Timed out for ${require("ms")(totalSeconds * 1000, {long: true})}`)
    .setTimestamp()]})
    }else{
