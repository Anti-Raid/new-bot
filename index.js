...
client.on(Events.MessageDelete, (message) => {
  ...
  message.client.channels.cache.get("1064256469010219008").send({
    embeds: [
      new EmbedBuilder()
        .setColor("Orange")
        .setDescription(
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (["⏮️", "◀️", "⏹️", "▶️", "⏭️"].includes(interaction.customId)) return;
    else {
      if (!button)
        return interaction.reply(
  ...
  if (interaction.isAutocomplete()) {
    ...
    try {
      ...
      return logger.error(`Autocomplete (${interaction.commandName})`, error);
    }
  }
});
